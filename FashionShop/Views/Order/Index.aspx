...
<asp:Content ID="StyleSheet" ContentPlaceHolderID="StyleSheetContent" runat="server">
    <link href="/Content/css/order/order-index.css" rel="stylesheet" />
</asp:Content>

<asp:Content ID="Script" ContentPlaceHolderID="ScriptContent" runat="server">
    <script src="/Content/js/order/order-index.js"></script>
</asp:Content>
    ...
    <article class="container" ng-controller="IndexCtrl">
        ...
        <ul class="orders">
            <li ng-repeat="order in orders">
                <div class="order">
                    <div>
                        <h5 class="order-time">{{showTime(order.Time)}}</h5>
                        <h4 class="order-name">{{order.CustomerName}}</h4>
                    </div>
                    <div class="div-buttons">
                        <core-icon-button icon="delete" class="button-delete" ng-click="deleteOrder(order)"></core-icon-button>
                        <core-icon-button icon="create" class="button-update" ng-click="updateOrder(order)"></core-icon-button>
                    </div>
                </div>
            </li>
        </ul>
